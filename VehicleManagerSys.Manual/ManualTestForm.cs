...
{
    public partial class ManualTestForm : FrmWithTitle
    {
        private VEHICLE_DISPATCH _DISPATCH = null;
        public ManualTestForm(VEHICLE_DISPATCH dispatch)
        {
            InitializeComponent();
        ...
        private void StartTest()
        {
            try
            {
                string xml = $"<?xml version=\"1.0\" encoding=\"GBK\"?><root><vehispara>" +
                    ...
                    $"<jyxm>DC</jyxm><kssj>{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}</kssj></vehispara></root>";
        ...
        private void EndTest()
        {
            try
            {
                string xml = $"<?xml version=\"1.0\" encoding=\"GBK\"?><root><vehispara><jylsh>{_DISPATCH.AJLSH}</jylsh>" +
                    $"<jyjgbh>1100000050</jyjgbh><jcxdh>1</jcxdh><jyxm>DC</jyxm><jycs>{_DISPATCH.AJJCCS}</jycs><hpzl>{_DISPATCH.HPZLDH}</hpzl>" +
        ...
        private void Capture(string zpzl)
        {
            try
            {
                ...
                string xml = $"<?xml version=\"1.0\" encoding=\"GBK\"?><root><vehispara><jylsh>{_DISPATCH.AJLSH}</jylsh><jcxdh>1</jcxdh>" +
                    ...
                    $"<gwjysbbh>2</gwjysbbh><jyxm>DC</jyxm><kssj>{DateTime.Now.ToString("yyyy-MM-dd HH: mm: ss")}</kssj><zpzl>{zpzl}</zpzl></vehispara></root>";
        ...
        private void SaveDC()
        {
            try
            {
                RESULT_CHASISS_MANUAL chasiss = new RESULT_CHASISS_MANUAL();
                chasiss.JCLSH = _DISPATCH.JCLSH;
                chasiss.DTDP_PD = "1";
                List<string> carIgnoreArr = (from p in chasiss.GetType().GetProperties()
                                             where p.GetValue(chasiss, null) == null || string.IsNullOrEmpty(p.GetValue(chasiss, null).ToString())
                                             select p.Name).ToList();
                if (!carIgnoreArr.Contains("ID")) carIgnoreArr.Add("ID");
                AppHelper.Db.InsertOrUpdate(chasiss, null, new string[] { "JCLSH" }, carIgnoreArr.ToArray());

                string sql = "UPDATE VEHICLE_DISPATCH SET YJXM = ISNULL(YJXM,'') + 'DC,' WHERE JCLSH = @JCLSH";
                Hashtable hashtable = new Hashtable();
                hashtable.Add("JCLSH", _DISPATCH.JCLSH);
                AppHelper.Db.ExcuteNonQuery(sql, hashtable);
            }
            catch (Exception ex)
            {
                ControlHelper.ThreadInvokerControl(this, () =>
